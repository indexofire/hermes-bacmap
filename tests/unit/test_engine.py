...
from hermes_bacmap.engine.utils import merge_intervals, confidence_tier, classify_allele  # noqa: E402


class TestMergeIntervals:
    def test_single_hit(self):
        h = Hit(query_id="q", subject_id="s", identity=99.0,
                subject_start=10, subject_end=109, alignment_length=100)
        cov, ident = merge_intervals([h], subject_length=1000)
        assert cov == 10.0
        assert ident == 99.0

    def test_overlapping_hits(self):
        h1 = Hit(query_id="q", subject_id="s", identity=99.0,
                 subject_start=1, subject_end=100, alignment_length=100)
        h2 = Hit(query_id="q", subject_id="s", identity=99.0,
                 subject_start=50, subject_end=150, alignment_length=101)
    ...
    def test_adjacent_hits_no_gap(self):
        h1 = Hit(query_id="q", subject_id="s", identity=99.0,
                 subject_start=1, subject_end=100, alignment_length=100)
        h2 = Hit(query_id="q", subject_id="s", identity=99.0,
                 subject_start=101, subject_end=200, alignment_length=100)
        cov, _ = merge_intervals([h1, h2], subject_length=1000)
        assert cov == 20.0
...
class TestClassifyAllele:
    ...
    def test_novel(self):
        label, score = classify_allele(98.0, 99.0,
                                        min_identity=95.0, min_coverage=98.0)
        assert label == "novel"
        assert score == 63
...
class TestHitToDict:
    def test_preserves_zero_values(self):
        h = Hit(query_id="q", subject_id="s", identity=0.0, bit_score=0.0,
                alignment_length=0)
        d = h.to_dict()
    ...
    def test_preserves_all_fields(self):
        ...
        d = h.to_dict()
        expected_keys = {
            "query_id", "subject_id", "identity", "query_coverage",
            "subject_coverage", "evalue", "bit_score", "query_start",
            "query_end", "subject_start", "subject_end", "strand",
            "alignment_length", "mismatches", "mapq", "backend",
        }
