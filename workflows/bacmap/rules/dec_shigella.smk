...
rule dec_ecoh_serotype:
    ...
    shell:
        ...
        "from hermes_bacmap.typing.ecoh_serotyper import serotype as ecoh_serotype; "
        "import json; r = ecoh_serotype('{params.contigs}'); "
...
rule shigella_serotype:
    ...
    shell:
        ...
        "{params.python} -c \""
        "import sys; sys.path.insert(0, '{params.src_path}'); "
        "from hermes_bacmap.typing.shigella_serotyper import serotype as shigella_serotype; "
        "import json; r = shigella_serotype('{params.contigs}'); "
        "json.dump(r.to_dict() if hasattr(r,'to_dict') else r, "
        "open('{params.out}', 'w'), ensure_ascii=False, indent=2)"
        "\" || echo '{params.fallback}' > {params.out}"
