...
```
                ...
                └─────────┬─────────┘
                          │  conda env / 容器化执行
                ┌─────────▼─────────┐
                │ Bioinformatics    │  ← spades/sistr/snippy/amrfinderplus/...
                │     Tools         │
                └─────────┬─────────┘
                          │  输出文件 + 元数据
                ┌─────────▼─────────┐
...
- `tool_versions`（spades/sistr/snippy 等工具的 conda 版本）
...
- **参考数据库版本化**：AMRFinderPlus / ResFinder / CARD / PubMLST 等数据库每次更新记录版本号，分析结果挂对应版本
- **SQLite 备份策略**：
...
| 4. 注释 | Bakta / Prokka | contigs FASTA | GFF/GenBank | ✅ |
...
| 8. 质粒 | PlasmidFinder | contigs FASTA | 质粒复制子 | ✅ |
| 9. SNP / 系统发育 | snippy + iqtree | clean FASTQ + reference | VCF + tree | V0.2 |
...
| Salmonella 模块（SeqSero2 + SISTR） | BacWORK | Salmonella 血清型 |
| ResFinder / AMRFinderPlus rule | BacWORK | 4 病原通用 AMR |
...
| 与 EIEC 鉴别 | ipaH 阳性 + ECTyper 血清型 | ✅ 组合判断 |
...
| 能力 | 工具 | Snakemake 集成 |
|---|---|---|
| O/K 血清型 | [VPsero](https://github.com/shengzheBian/VPsero) + [Kaptive Vp 库](https://github.com/aldertzomer/vibrio_parahaemolyticus_genomoserotyping) | ✍️ **完全新写**（无现成 pipeline） |
| 毒力（tdh/trh/tlh/toxR） | abricate 自定义库 | ✍️ 新写 |**这是 Hermes-bacmap 的差异化价值点**——BacWORK 显式声明 *V. parahaemolyticus* "no specific steps"，本项目补全生态空白。
...
| snippy（SNP calling） | 4 min | 8-16 GB | ~1 小时 |
| AMRFinderPlus + abricate | 3-4 min | < 4 GB | ~30 min |
| mlst + SISTR + SeqSero2 | < 1 min | < 1 GB | < 10 min |

**关键瓶颈**：SPAdes 装配。**优化建议**：纯监测场景（不需 de novo）可跳过 SPAdes，仅用 snippy 做 reference-based SNP calling，吞吐翻 3-5 倍。
...
Layer 2: Deterministic Verifier（确定性规则校验）
  - AMR 基因必须在 CARD/NCBI AMRFinderPlus 数据库（exact match）
...
| 中等任务（单株 mlst/AMRFinderPlus） | 5-30 min | Hermes 后台调用 + 轮询完成 |
...
Snakemake 失败时 Hermes 解析 log，调用 LLM 识别 root cause：

- OOM → 调小 `--resources mem_mb` 或减少并发
- 工具版本不兼容 → 更新 conda env
- 参考数据库缺失 → 提示下载
- 测序数据质量差 → 提示用户重新测序或降级模式

参考实现：[PipelineAgentService](https://github.com/gustaveroussy/PipelineAgentService) 已有 Nextflow 自诊断模式，本项目借鉴思想应用到 Snakemake。
...
| 指标 | 目标 | 对比基线 |
|---|---|---|
| AMR 检测灵敏度 | ≥ 95% | vs [AMRFinderPlus 论文](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7665300/) |
| AMR 检测特异性 | ≥ 98% | 同上 |
...
| **工具升级回归** | spades/sistr/snippy 升级时 | gold standard 套件 | AMR/MLST/血清型结果差异 < 1% |
...
┌─────────────────────────────────────────────────────────────────┐
│ V0.2 — ✅ 完成                                                     │
│ 扩展 DEC（ECTyper + uidA + pathotype）+ Shigella（ipaH）— 10 株验证通过
...
- **ECTyper**（DEC）: [github.com/phac-nml/ecoli_serotyping](https://github.com/phac-nml/ecoli_serotyping)
- **PathotypeR**（DEC pathotype）: [github.com/kevinsblake/PathotypeR](https://github.com/kevinsblake/PathotypeR)
- **ShigEiFinder**（Shigella）: [github.com/LanLab/ShigEiFinder](https://github.com/LanLab/ShigEiFinder)
- **VPsero**（V. parahaemolyticus）: [github.com/shengzheBian/VPsero](https://github.com/shengzheBian/VPsero)
- **Kaptive Vp 库**: [github.com/aldertzomer/vibrio_parahaemolyticus_genomoserotyping](https://github.com/aldertzomer/vibrio_parahaemolyticus_genomoserotyping)
