...
Hermes-bacmap 以 [Hermes Agent](https://github.com/NousResearch/hermes-agent) 为编排核心，将 Snakemake 工作流、23 个生信工具、SQLite 数据模型与本地 LLM 推理整合为统一平台。用户用中文或英文自然语言即可完成从 FASTQ 上传到暴发溯源的全流程。
...
| 维度 | 数量 | 说明 |
|---|---|---|
| Hermes Tools | **23** | 8 个生信原语 + 15 个高层分析工具 |
| Snakemake Rules | **23** | per-sample DAG + cohort SNP DAG（3 物种组） |
| 测试用例 | **193** | GOM + 溯源索引 + Verifier + Engine + Utils，全绿 |
