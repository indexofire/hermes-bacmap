...
Hermes-bacmap 注册 **24 个 Tools**，分为两类：8 个生信原语（底层算法封装）与 10 个高层分析工具（业务级流程）。所有 tool 在 `src/hermes_bacmap/tools.py` 实现 handler，`schemas.py` 定义 JSON Schema。
...
- [Snakemake 管线](../architecture/pipeline.md) — `bio_analyze_pathogen` 触发的 22 条规则
