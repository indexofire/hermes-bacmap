...
Hermes-bacmap 注册 **24 个 Tools**，分为两类：8 个生信原语（底层算法封装）与 16 个高层分析工具（业务级流程）。所有 tool 在 `src/hermes_bacmap/tools.py` 实现 handler，`schemas.py` 定义 JSON Schema。
...
| Tool | 功能 | 输入 | 输出 |
|---|---|---|---|
| `bio_analyze_pathogen` | 触发 Snakemake 全流程（跨病原自动路由） | `sample_id` | `{sample}_summary.json` |
...
| `bio_validate_taxonomy` | 物种鉴定（双模式） | `sample_id`, `mode` | completeness / contamination / gtdb_taxonomy |
| `bio_diagnose` | 诊断管线失败 | `log_path` 或 `stderr_text` | 错误类型 / 根因 / 影响规则 / 修复命令 |
| `bio_vpa_serotype` | V. parahaemolyticus O/K 血清型预测 | `contigs` 路径 | 血清型（如 O3:K6）+ 置信度 + coverage / identity |
| `bio_add_metadata` | 录入/更新菌株背景元数据 | `sample_id` + 患者/分离/暴发字段 | 元数据记录 |
...
底层由 `gene_scanner.py`（546 行）驱动，委托 [engine.SequenceMatcher](../architecture/engine.md)。检查 BLAST 返回码，非零 raise（防止静默假阴性）。
...
- [Snakemake 管线](../architecture/pipeline.md) — `bio_analyze_pathogen` 触发的 24 条规则
